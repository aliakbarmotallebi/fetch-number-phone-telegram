...
class Sql
{
	...
	public function __construct()
	{
		...
		$db['database']	= 'tgbot';	// DataBase Name
		$db['username']	= 'root';	// Mysql User Name	: root
		$db['password']	= 'akbar';	// Mysql Password
    ...
    function SetLog($log,$token,$type = null)
    {
        $LogInsert['id']          =   'null';
        $LogInsert['log']    =   $log;
		$LogInsert['token']  =   $token;
        $LogInsert['type']      =   $type;
	...
	public function RequestToServer($url , $data , $bg=false){
		...
		$curl = curl_init($url);
		$curl_post_data = array("data" => json_encode($data));
		//var_dump($curl_post_data);
			...
			curl_close($curl);
			$original_array=unserialize($curl_response);
