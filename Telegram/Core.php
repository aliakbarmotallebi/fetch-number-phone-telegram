...
function ErrorHandel($errno, $errstr, $errfile, $errline , $errcontext) {
    $Fileds = array('level'=>$errno ,'message'=>$errstr , 'file'=>$errfile , 'line'=>$errline , 'context'=>'d');
    $sql=new Sql();
    $sql->Insert('error_log', $Fileds);
}
$errors = set_error_handler("ErrorHandel");
...
class Core extends Sql
{
    ...
    */
    public function SetPid(){
        ...
        $InsertPid = $this->Select('php' , 'status = 1');
        if($InsertPid){
            exec("kill $InsertPid[procId]");
            $data['status']=0 ;
            $this->Update('php' ,$data , ' `id` = "'.$InsertPid['id'].'"');
        }
        $data['procId']=getmypid();
        $data['status']=1;
        $this->Insert('php',$data);
    }
    ...
    */
    public function RemovePid(){
        $this->SetLog("RemovePid","dd","reomve");
        $SelectPid = $this->Select('php' , 'status = 1');
        if($SelectPid){
            $data['status']=0 ;
            $this->Update('php' ,$data , ' `id` = "'.$SelectPid['id'].'"');
            exec("kill $SelectPid[procId]");
        }
        $this->Response(True , "Stop");
    }
